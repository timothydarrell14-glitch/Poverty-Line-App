...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/jobs?status=open"), { signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) throw new Error("Unable to load job opportunities.");
        const data = await response.json();
        setJobs((data.jobs ?? []).map((job) => ({
          ...job,
          type: job.status || "Open",
          location: "Partner organisation",
  ...
  const submitAccount = async (event) => {
    ...
    try {
      if (isSignUp) {
        const signup = await fetch(apiUrl("/api/users/register"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ first_name: firstName, last_name: lastName, email, password }) });
      ...
      const login = await fetch(apiUrl("/api/users/login"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, password }) });
  ...
  const submitApplication = async (event) => {
    ...
    setLoading(true); setMessage("");
    try { const response = await fetch(apiUrl("/api/job-applications"), { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ job_id: selectedJob.job_id }) }); const data = await readResponse(response); if (!response.ok) throw Error(errorMessage(data, "Your application could not be submitted.")); setView("success"); showToast("Job application submitted successfully.", "success"); } catch (error) { setMessage(error.message); showToast(error.message || "Your application could not be submitted.", "error"); } finally { setLoading(false); }
  };
