...
def test_atomic_community_creation(client):
	...
	membership = CommunityMembership.query.filter_by(community_id=data["community_id"]).first()
...
def test_create_and_list_posts(client):
	...
	p_res = client.post(f"/api/communities/{c_id}/posts", json={"content": "Hello community!"})
