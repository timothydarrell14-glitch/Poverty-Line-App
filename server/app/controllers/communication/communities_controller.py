...
def list_communities():
    ...
    for c in communities:
        member_count = CommunityMembership.query.filter_by(
...
def get_community(community_id):
    ...
    for m in memberships:
        if m.user:
            ...
            avatar = (
                m.user.avatar_url or f"https://i.pravatar.cc/150?u={m.user.user_id}"
            )
            ...
            if m.role in ["admin", "moderator"]:
                moderators.append(
                    {"user_id": m.user.user_id, "name": display_name, "role": m.role}
                )
...
def list_community_channel_posts(community_id):
    db.get_or_404(Community, community_id)
...
def join_community_channel(community_id):
    ...
    if existing:
        return jsonify(
            {"message": "Already a member", "membership_id": existing.membership_id}
        ), 200
